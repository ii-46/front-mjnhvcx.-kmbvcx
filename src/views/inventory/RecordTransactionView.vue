...
const transactionStore = useTransactionTypeStore()
...
const initForm: {
  ...
  transactionType: null | RecordModel,
} = {
...
watch(formData, async () => {
  ...
  if (formData.receiver !== null && formData.toInventory === null) {
    const inventory = inventoryStore.inventory.filter(i => i.user_id === formData.receiver!.id)[0]
    formData.toInventory = inventory ?? null
  }
  if (formData.assigner !== null && formData.fromInventory === null) {
    const inventory = inventoryStore.inventory.filter(i => i.user_id === formData.assigner!.id)[0]
    formData.fromInventory = inventory ?? null
  }
}, {
  deep: true,
  immediate: true,
...
onMounted(async () => {
  ...
  await transactionStore.listenToTransactions()
})

onUnmounted(async () => {
  await usersStore.unsubUsers()
  await inventoryStore.unsubInventory()
  await transactionStore.unsubTransactions()
})
...
async function onSubmit() {
  ...
  const body = {
    ...
    transaction_type_id: formData.transactionType!.id
  }
  ...
  try {
    ...
    toast.success(t("record-transaction-devices-successfully"))
    onResetForm()
  } catch (e) {
    toast.error(e)
  } finally {
  ...
  <v-app>
    <v-container>
      ...
      <div class="flex justify-between">
        <h1 class="text-2xl font-semibold py-3">{{ $t("record-transaction") }}</h1>
      </div>
      <v-card>
            ...
            <v-row>
                ...
                <v-autocomplete
                    ...
                    :items="inventoryStore.inventory.filter(i => {
                      if(formData.toInventory) {
                        return  i.id !== formData.toInventory.id
                      }
                      return true
                    }
                    )"
              ...
              <v-col sm="12" md="12" lg="2" align-self="center">
                <v-autocomplete
                    ...
                    :items="transactionStore.transactionType"
                    return-object
                    item-title="name"
                    :rules="[rules.required()]"
                ></v-autocomplete>
              ...
              <v-col sm="12" md="12" lg="5">
                ...
                <v-autocomplete
                    ...
                    :items="inventoryStore.inventory.filter(i => {
                      ...
                      // }
                      if(formData.fromInventory) {
                        return  (i.id !== formData.fromInventory.id) // check if not from opposite
                      }
                      return true
                    }
                    )"
